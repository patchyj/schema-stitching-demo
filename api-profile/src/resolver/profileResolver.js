...
const resolvers = {
  ...
  Mutation: {
    ...
    addProfile: async (parent, profile, context) => {
      if (!context.user)
        throwError("AUTH", "You must be a member to add a post");

      if (profile.user !== context.user.id)
        throwError("AUTH", "A profile can only be created by the user");

      const existingProfile = await Profile.findOne({ user: profile.user });

      if (existingProfile !== null)
        throwError("USER", "A profile already exists for this user");
    ...
    updateProfile: async (parent, profile, context) => {
      if (!context.user)
        throwError("AUTH", "You must be a member to update this profile");
      ...
      if (profile.user !== context.user.id)
        throwError("AUTH", "Only the user can update this profile");

      const userProfile = await Profile.findOne({ user: profile.user });

      if (profile.user !== context.user.id)
        throwError("USER", "A profile doesn't exist for this User");
    ...
    deleteProfile: async (parent, { id, user }, context) => {
      if (!context.user)
        throwError("AUTH", "You must be a member to delete this profile");
      ...
      if (user !== context.user.id)
        throwError("AUTH", "Only the user can delete this profile");

      await profile.remove();

      const isProfileDeleted = await Profile.findById(id);

      const message = isProfileDeleted
